...
this.setState({
    ...
    render() {
        ...
        let dis=this.props.message.reply=="" || this.state.staus==200?<input style={{width:"100%",borderRadius:"12px"}} className="form-control" id="replyto" onChange={this.reply} type="text" placeholder="input here..." value={this.state.reply} />:<input style={{width:"100%",borderRadius:"12px"}} className="form-control" id="replyto" onChange={this.reply} type="text" placeholder="input here..." value={this.state.reply} disabled/>

        let butondis=jwt.decode(localStorage.getItem('jwtToken')).UserType=="owner"?<button onClick={this.sendReply}><img style={{height:"40px"}} src={Logo2}></img></button>:null;
