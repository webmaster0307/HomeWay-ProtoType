...
import { addBookMutation ,updateTravelerProfile} from '../mutation/mutations';
...
//     }
//   })
console.log("traveler",typeof(cookie.load('traveler')));
let uname=cookie.load('traveler');
var profile=this.props.updateTravelerProfile({
  variables: {
...
componentDidMount(){
  if(cookie.load('traveler')){
    console.log("traveler cookie",cookie.load('traveler'));
    var data={
    ...
    .then(res=>{
      ...
      let imagePreview = res.data.photo?'data:image/jpg;base64, ' + res.data.photo:null;
...
export default compose(
  ...
  graphql(updateTravelerProfile, { name: "updateTravelerProfile" })
)(Profile);
