...
import { addBookMutation ,updateTravelerProfile,fetchproperties} from '../mutation/mutations';
...
this.state={
 ...
 downloadData(){
    ...
    var properties=this.props.fetchproperties({
        variables: {
            address:this.state.location,
            accomodates:this.state.guests
        ...
        let temp = JSON.stringify(res.data.propertySearch);
       temp = JSON.parse(temp);
        this.setState({
            properties:temp
        })
...
//   }
  ...
  render(){
      console.log("fetched Properties",this.state.properties)
...
export default graphql(fetchproperties, { name: "fetchproperties" })
  (ImgMediaCard);
